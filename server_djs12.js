      ...
      var links_embed = {
        embed: {
          ...
          fields: [
            {
              name: "Discord-сервер",
              value: "https://discord.gg/9husXQy"
            },
            {
              name: "YouTube",
            ...
            },
            {
              ...
              value:
                "https://discordapp.com/oauth2/authorize?client_id=634271325057318943&permissions=8&scope=bot"
            }
          ]
        }
      };
...
client.on("message", message => {
  if (message.author === client.user) return;
  if (
    message.content.startsWith(prefix + "photograph") ||
    message.content.startsWith(prefix_a + "photograph") ||
    message.content.startsWith(prefix_b + "photograph") ||
    message.content.startsWith(prefix_c + "photograph")
  ) {
    var t_log = {
      embed: {
        color: 0xff8800,
  ...
  if (
    ...
    if (blockid === message.author.id) {
      ...
      if (message.guild.afkChannel && message.guild.afkChannel.name) {
        afkCh =
          message.guild.afkChannel.name + " (" + message.guild.afkTimeout + ")";
      } else {
        afkCh = "Отсутствует";
      }
      client.channels.cache.get("564022728143929370").send(t_log);
      ...
      client.users.fetch(id).then(user => {
        user.send(
          message.author.id +
            " или " +
            message.author.tag +
            " хочет узнать инфу о сервере, в которой он сидит. Нормально, а че?"
        );
      });
    }
  }
  ...
  if (message.content.startsWith(prefix + "help_srv")) {
    ...
    var chsname_embed = {
      embed: {
        ...
        footer: {
          text:
            "Последние изменения: 5 июня 2020 г., 13:08 МСК | Часть 1. Наименования каналов"
        }
      }
    };
    var rlsname_embed = {
      ...
      client.users.fetch(id).then(user => {
        user.send(
          message.author.id +
            " или " +
            message.author.tag +
            " хочет узнать о пожертвовании или просто задонатить. ;)"
        );
      });
    }
  }
